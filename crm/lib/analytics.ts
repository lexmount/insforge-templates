...
export function initializeAnalytics(): void {
  ...
  const token = runtime?.posthogProjectToken || process.env.NEXT_PUBLIC_POSTHOG_PROJECT_TOKEN;
  if (process.env.NODE_ENV !== 'production' || !token || posthog.__loaded) return;

  const configuredRate = Number(process.env.NEXT_PUBLIC_POSTHOG_REPLAY_SAMPLE_RATE ?? '0.1');
  const replaySampleRate = Number.isFinite(configuredRate)
    ? Math.min(1, Math.max(0, configuredRate))
    : 0.1;

  posthog.init(token, {
    api_host: runtime?.posthogHost || process.env.NEXT_PUBLIC_POSTHOG_HOST || 'https://us.i.posthog.com',
    autocapture: false,
...
export function identify(userId: string): void {
  if (!posthog.__loaded || !userId || EMAIL_VALUE.test(userId)) return;
