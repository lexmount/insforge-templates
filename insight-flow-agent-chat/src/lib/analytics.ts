...
const PII_KEY =
  /(^|_)(email|e_mail|name|first_name|last_name|phone|mobile|address|password|token|secret|authorization|cookie|message|prompt|content|query)(_|$)/i;
...
export function initializeAnalytics(): void {
  ...
  posthog.init(token, {
    api_host:
      runtime?.posthogHost || import.meta.env.VITE_POSTHOG_HOST || 'https://us.i.posthog.com',
    autocapture: false,
    ...
    loaded(client) {
      client.register(
        safeProperties({
          ...
          template_version_id:
            runtime?.templateVersionId || import.meta.env.VITE_INSFORGE_TEMPLATE_VERSION_ID,
