create table if not exists public.insight_flow_agent_configs (
  ...
  check (
    ...
    length(encrypted_api_key) <= 2048 and length(api_key_iv) <= 256 and
    length(api_key_last_four) between 1 and 4
  )
);
...
grant update (base_url, target_mode, target, disable_tools, encrypted_api_key, api_key_iv, api_key_last_four)
